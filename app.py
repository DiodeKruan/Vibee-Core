...
def filter_data(df: pd.DataFrame, params: dict) -> pd.DataFrame:
  ...
  if data_source == "traffy":
    ...
    # Longdo data - filter by event_types
    selected_event_types = params.get("event_types", [])

    if selected_event_types and "type" in filtered.columns:
...
def render_fullscreen_map(data: pd.DataFrame, layer_type: str, layer_params: dict, data_source: str = "traffy"):
  ...
  # Tooltip config - different for each data source
  tooltip = None
  if layer_type in ["scatter"]:
    if data_source == "traffy":
      ...
      # Longdo events tooltip
      tooltip = {
          "html": """
                    <div style="background:rgba(15,15,20,0.95);padding:12px 16px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);font-family:system-ui;max-width:320px">
                        <div style="color:#f97316;font-weight:600;font-size:13px;margin-bottom:6px;text-transform:capitalize;">{type}</div>
                        <div style="color:#e2e8f0;font-size:13px;font-weight:500;margin-bottom:6px;">{title}</div>
                        <div style="color:#94a3b8;font-size:12px;line-height:1.4;margin-bottom:6px;">{description}</div>
                        <div style="color:#64748b;font-size:11px;margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,0.1);">
                            <div>📍 {coords_display}</div>
                            <div style="margin-top:4px;">👤 {posted_by}</div>
                        </div>
                    </div>
                """,
          "style": {"backgroundColor": "transparent", "color": "white"},
...
def render_floating_header(total: int, filtered: int, categories: int, layer: str, data_source: str = "traffy"):
  """Render the floating header with stats."""
  source_label = "Traffy Fondue" if data_source == "traffy" else "Longdo Events"
  st.markdown(
      f"""
        <div class="floating-header">
            <div class="title-section">
                <div class="main-title">Bangkok Urban Reports</div>
                <div class="subtitle">{source_label} Analytics</div>
            </div>
            <div class="stats-row">
                <div class="stat-item">
  ...
  try:
    if data_source == "traffy":
      ...
      if not df.empty:
        ...
        # Add coords_display for tooltip
        if "lat" in df.columns and "lon" in df.columns:
          df["coords_display"] = df.apply(
              lambda row: f"{row['lat']:.4f}, {row['lon']:.4f}", axis=1
          )

        # Fill NaN values for tooltip display
...
def main():
  ...
  if data_source == "traffy":
    type_count = len(params.get("categories", []))
  else:
    type_count = len(params.get("event_types", []))
