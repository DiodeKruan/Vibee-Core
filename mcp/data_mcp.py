...
class DataQuery:
  ...
  data_source: str = "traffy"  # 'traffy' or 'longdo'
...
class DataMCP:
  ...
  def _build_filters(self, query: DataQuery) -> Dict[str, Any]:
    ...
    # Handle Traffy categories or Longdo event_types based on data source
    if query.data_source == "longdo":
      if query.event_types:
        filters["event_type"] = query.event_types
    else:
      if query.categories:
  ...
  def _handle_schema(self, query: DataQuery) -> DataResponse:
    """Handle schema query."""
    try:
      from data.queries import get_schema_info, get_longdo_schema_info

      if query.data_source == "longdo":
        schema = get_longdo_schema_info()
      else:
  ...
  def _handle_aggregation(self, query: DataQuery) -> DataResponse:
    ...
    try:
      from data.queries import query_aggregation, query_longdo_aggregation
      ...
      if query.data_source == "longdo":
        results = query_longdo_aggregation(
  ...
  def _handle_statistics(self, query: DataQuery) -> DataResponse:
    ...
    try:
      from data.queries import query_statistics, query_longdo_statistics

      filters = self._build_filters(query)

      if query.data_source == "longdo":
        stats = query_longdo_statistics(
  ...
  def _handle_time_series(self, query: DataQuery) -> DataResponse:
    ...
    try:
      from data.queries import query_time_series, query_longdo_time_series
      ...
      if query.data_source == "longdo":
        results = query_longdo_time_series(
